    ...
    public class CriteriaService : ICriteriaService
    {
        ...
        public async Task<ServiceResult> GetCriteriaAsync(Guid roundId)
        {
            ...
            var criteria = await _context.Criteria
                ...
                .Select(c => new CriteriaDetailDto
                {
        ...
        public async Task<ServiceResult> CreateCriteriaAsync(Guid roundId, CreateCriteriaRequest request)
        {
            ...
            if (currentTotalWeight + request.Weight > 100)
                return ServiceResult.BadRequest(new { message = "Total criteria weight cannot exceed 100." });
            ...
            if (duplicate)
                return ServiceResult.BadRequest(new { message = "Criteria name already exists in this round." });
        ...
        public async Task<ServiceResult> UpdateCriteriaAsync(Guid roundId, Guid criteriaId, UpdateCriteriaRequest request)
        {
            ...
            if (criteria == null)
                return ServiceResult.NotFound(new { message = "Criteria not found." });
            ...
            return ServiceResult.Ok(new { message = "Criteria updated successfully." });
        }
        ...
        public async Task<ServiceResult> DeleteCriteriaAsync(Guid roundId, Guid criteriaId)
        {
            ...
            if (criteria.Scores.Any())
                return ServiceResult.BadRequest(new { message = "Cannot delete criteria because it already has scores." });

            _context.Criteria.Remove(criteria);
            await _context.SaveChangesAsync();

            return ServiceResult.Ok(new { message = "Criteria deleted successfully." });
        }
    }
}
