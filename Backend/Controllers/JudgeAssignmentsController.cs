...
namespace SEAL.NET.Controllers
{
    ...
    public class JudgeAssignmentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAssignments()
            => Ok(await _judgeAssignmentService.GetAssignmentsAsync());

        [HttpPost]
        public async Task<IActionResult> CreateAssignment([FromBody] CreateJudgeAssignmentRequest request)
            => (await _judgeAssignmentService.CreateAssignmentAsync(request)).ToActionResult(this);

        [HttpDelete("{assignmentId}")]
        public async Task<IActionResult> DeleteAssignment(Guid assignmentId)
            => (await _judgeAssignmentService.DeleteAssignmentAsync(assignmentId)).ToActionResult(this);
    }
}
